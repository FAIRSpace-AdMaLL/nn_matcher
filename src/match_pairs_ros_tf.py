...
from models.utils_naive import (make_matching_plot, AverageTimer, image2tensor)
import time
...
class NN_Matching:
    ...
    def __init__(self):
        ...
        parser.add_argument('--resize', type=int, default=[400, 300], help='Resize the input image before running inference. If -1, do not resize')
        ...
        parser.add_argument('--mask', type=float, default=1.0, help='Create a mask to get ride of ground.')
        # V-T&R parameters
    ...
    def matching_pair(self, req):
        ...
        print("psotprocessing: "+ str((time.time()-now)))
        now = time.time()
        ...
        print("matching:  "+ str((time.time()-now)))
        now = time.time()
