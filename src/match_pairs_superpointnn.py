...
if __name__ == '__main__':
    ...
    matching = Matching(config, destcriptor_only=opt.descriptor_only).eval().to(device)
    ...
    for i, pair in enumerate(pairs):
        ...
        viz_path = output_dir / '{}_{}_matches_{}.{}'.format(stem0, stem1, i, opt.viz_extension)
        ...
        if do_match:
            ...
            if opt.descriptor_only is True:
                desc0, desc1 = pred['descriptors0'], pred['descriptors1']
                desc0 = np.transpose(desc0)
                desc1 = np.transpose(desc1)
                mkpts0, mkpts1, conf, matches = match_descriptors(kpts0, desc0, kpts1, desc1)
                mconf = conf
